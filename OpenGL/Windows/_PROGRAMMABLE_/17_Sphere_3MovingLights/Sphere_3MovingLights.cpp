	...
	const GLchar* vertexShaderSourceCode =
		...
		"{" \
				...
				"{" \
						...
						"phong_ads_light = redLight + blueLight + greenLight;" \
				"}" \
				"else" \
				"{" \
...
void uninitialize(void)
{
	if (vbo_normals_sphere)
	{
		glDeleteVertexArrays(1, &vbo_normals_sphere);
		vbo_normals_sphere = 0;
	}
...
{ 
	...
	lightPosition_Blue[0] = 100.0f*cos(angle_X);
	lightPosition_Blue[1] = 0.0f;
	lightPosition_Blue[2] = 100.0f*sin(angle_X);
	lightPosition_Blue[3] = 1.0f;

	lightPosition_Green[0] = 100 * cos(angle_X);
	lightPosition_Green[1] = 100 * sin(angle_X);
