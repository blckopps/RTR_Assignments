package com.example.ColTwoShapes;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer ,OnDoubleTapListener,OnGestureListener
{
	...
	private int[] vbo_col_triangle = new int[1];
	private int[] vbo_col_rectangle = new int[1];
	...
	public boolean onTouchEvent(MotionEvent event)
	{
	 ...
	 private void initialize()
	 {
		...
		final float[] rectangle_col = new float[]
					{
							1.0f, 1.0f, 0.0f,
							1.0f, 1.0f, 0.0f,
							1.0f, 1.0f, 0.0f,
		...
		//col buffer triangle
		GLES31.glGenBuffers(1, vbo_col_triangle, 0);

		 GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, vbo_col_triangle[0]);

		 ByteBuffer byteBufferCol = ByteBuffer.allocateDirect(triangle_col.length * 4);

		 byteBufferCol.order(ByteOrder.nativeOrder());

		 FloatBuffer colBuffer = byteBufferCol.asFloatBuffer();

		 colBuffer.put(triangle_col);

		 colBuffer.position(0);
                
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER,
				    triangle_col.length * 4,
				    colBuffer,
				    GLES31.GL_STATIC_DRAW);
		...
		//color rectangle
		GLES31.glGenBuffers(1, vbo_col_rectangle, 0);

		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, vbo_col_rectangle[0]);

		//
		ByteBuffer byteBufferColRect = ByteBuffer.allocateDirect(rectangle_col.length * 4);

		byteBufferColRect.order(ByteOrder.nativeOrder());

		FloatBuffer bufferRectCol = byteBufferColRect.asFloatBuffer();

		bufferRectCol.put(rectangle_col);

		bufferRectCol.position(0);
		
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER,
					rectangle_col.length * 4,
					bufferRectCol,
					GLES31.GL_STATIC_DRAW );
	...
	private void display()
	{
		...
		Matrix.translateM(modelViewMatrix, 0 ,translateMatrix, 0, 2.1f, 0.0f, -9.0f);
		...
		Matrix.translateM(modelViewMatrix, 0 ,translateMatrix, 0, -1.6f, 0.0f, -9.0f);
