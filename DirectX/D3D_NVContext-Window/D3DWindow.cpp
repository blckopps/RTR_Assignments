...
HRESULT initialize(void)
{
	...
	*/
	...
	for (unsigned int iIndexAdapter = 0; iIndexAdapter < vAdapters.size(); iIndexAdapter++)
	{
		...
		size_t obtainedVideoMemory = desc.DedicatedVideoMemory;
		// Selection as per heighest video memory
		if ((desc.DedicatedVideoMemory != 0) && (obtainedVideoMemory > bestVideoMemory))
		{
	...
	{
		fprintf_s(gpFile, "Adapter type is : %S\n", desc.Description);
		fclose(gpFile);
	}
...
void display(void)
{
	...
	///First zero : default value 
	//do u want to match dispalyed fram refresh rate with monitors refresh rate
	//0, 1,2,3,4 you can give
	///Second zero: from which buffer fram should be displayed.
	//zero to show all
