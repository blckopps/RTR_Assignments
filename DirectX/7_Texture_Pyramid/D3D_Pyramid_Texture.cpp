...
HRESULT initialize(void)
{
	...
	*/
	...
	//Load texture
	hr = LoadD3DTexture(L"stone.bmp", &gpID3D11ShaderResouceView_Texture_Pyramid);
	if (FAILED(hr))
	{
		Log("LoadD3DTexture: failed for pyramid\n");
	}
	else
	{
		Log("LoadD3DTexture Successfull for pyramid\n");
	}
	...
	hr = gpID3D11Device->CreateSamplerState(&sampleDesc, &gpID3D11SamplerState);
	if (FAILED(hr))
	{
