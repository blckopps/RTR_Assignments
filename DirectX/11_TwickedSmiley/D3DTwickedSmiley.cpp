...
ID3D11ShaderResourceView *gpID3D11ShaderResouceView_White_Texture = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("D3D TWICKED SMILEY-SHUBHAM"),
...
HRESULT initialize(void)
{
	...
	HRESULT LoadD3DWhiteTexture(ID3D11ShaderResourceView **ppID3D11ShaderResourceView);
	...
	*/
	...
	bufferDesc_texture.ByteWidth = sizeof(float)*ARRAYSIZE(texcoords_full);
	...
	memcpy(mappedSubresource_texture.pData, texcoords_full, sizeof(texcoords_full));
	...
	{
		Log("LoadD3DTexture Successfull \n");
	}
	//Load White texture
	hr = LoadD3DWhiteTexture(&gpID3D11ShaderResouceView_White_Texture);
	if (FAILED(hr))
	{
		Log("LoadD3D white Texture: failed \n");
	}
	else
	{
		Log("LoadD3D white Texture Successfull \n");
	}
...
void display(void)
{
	...
	if (choice == 1)
	{
		...
		memcpy(mappedSubresource_texture.pData, texcoords_LB, sizeof(texcoords_LB));
	...
	else if (choice == 2)
	{
		...
		memcpy(mappedSubresource_texture.pData, texcoords_double, sizeof(texcoords_double));
	...
	else if (choice == 3)
	{
		...
		memcpy(mappedSubresource_texture.pData, texcoords_small, sizeof(texcoords_small));
	...
	else if (choice == 4)
	{
		...
		gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResouceView_White_Texture);
...
{
	HRESULT hr = NULL;
	//GLubyte whiteImage[whiteImageWidth][whiteImageHeight][4];
	byte whiteImage[64][64][4];
	int i, j, c;
	...
	for (i = 0; i < 64; i++)
	{
		for (j = 0; j < 64; j++)
		{
			...
			whiteImage[i][j][0] = (byte)255;
			whiteImage[i][j][1] = (byte)255;
			whiteImage[i][j][2] = (byte)255;
			whiteImage[i][j][3] = (byte)255;
		}
	}

	UINT imageBufferSize = 64 * 64 * 4;

	BITMAPINFOHEADER BitmapInfoHeader;
	ZeroMemory(&BitmapInfoHeader, sizeof(BitmapInfoHeader));
	BitmapInfoHeader.biBitCount = 32;    // 4 bit each RGBA  
	BitmapInfoHeader.biHeight = 64;
	BitmapInfoHeader.biPlanes = 1;
	BitmapInfoHeader.biSize = sizeof(BitmapInfoHeader);
	BitmapInfoHeader.biWidth = 64;
	...
	BitmapFileHeader.bfSize = imageBufferSize + sizeof(BitmapFileHeader) + sizeof(BitmapInfoHeader);
	...
	UINT BitmapImageSize = sizeof(BitmapFileHeader) + sizeof(BitmapInfoHeader) + sizeof(whiteImage);

	byte *bitmap = (BYTE*)malloc(BitmapImageSize);

	memcpy_s(bitmap, BitmapImageSize, &BitmapFileHeader, sizeof(BitmapFileHeader));
	memcpy_s(bitmap + sizeof(BitmapFileHeader), BitmapImageSize, &BitmapInfoHeader, sizeof(BitmapInfoHeader));
	memcpy_s(bitmap + sizeof(BitmapFileHeader) + sizeof(BitmapInfoHeader), BitmapImageSize, whiteImage, sizeof(whiteImage));

	hr = DirectX::CreateWICTextureFromMemory(gpID3D11Device, gpID3D11DeviceContext, (const uint8_t*)bitmap, (size_t)BitmapImageSize, NULL, ppID3D11ShaderResourceView, NULL);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "DirectX::CreateWICTextureFromMemory Failed .\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "DirectX::CreateWICTextureFromMemory Completed .\n");
		fclose(gpFile);
