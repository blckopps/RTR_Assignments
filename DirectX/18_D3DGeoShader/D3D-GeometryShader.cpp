...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("D3D GEOMETRY SHADER-SHUBHAM"),
...
HRESULT initialize(void)
{
	...
	*/
	...
	ID3DBlob *pID3DBlob_GeoShader = NULL;
	pID3DBlob_Error = NULL;
	...
	if (FAILED(hr))
	{
		Log("D3DCompile Geometry shader: Fail\n");
	}
	else
	{
		Log("D3DCompile Geometry successfull\n");
	}

	hr = gpID3D11Device->CreateGeometryShader(
		pID3DBlob_GeoShader->GetBufferPointer(),
		pID3DBlob_GeoShader->GetBufferSize(),
		NULL,
		&gpID3D11GeometryShader);

	if (FAILED(hr))
	{
		Log("CreateGeometryShader: Fail\n");
	}
	else
	{
		Log("CreateGeometryShader successfull\n");
	}
